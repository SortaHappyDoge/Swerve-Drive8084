...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  public ElevatorSubsystem(/*ArmSubsystem armSubsystem*/) {
    ...
    m_elevatorSparkMax0 = new SparkMax(10, MotorType.kBrushless);
    m_elevatorSparkMax1 = new SparkMax(11, MotorType.kBrushless);
    ...
    m_elevatorSparkMax1.configure(kElevatorSparkMaxConfig, ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
    ...
    m_elevatorPIDController.setTolerance(0.01);
    m_elevatorPIDController.setTolerance(0.01);
  ...
  public void periodic() {
    ...
    if (armBlocked && manualControl)
      doMove = false;
    else
      doMove = true;*/
    moveElevatorTo(desiredElevatorPosition);

    if (elevatorPosition > 0.25 && elevatorPosition > desiredElevatorPosition - 0.03)
      atTarget = true;
    else
      atTarget = false;
  ...
  public void moveElevatorTo(double m) {
    ...
    m = MathUtil.clamp(m, 0, 1.42);
    
    /*if (desiredElevatorPosition > 0.05 || elevatorPosition > 0.05) {
      m_armSubsystem.armMinMaxRotation = new double[] { ModuleConstants.kArmMinBlockedMaxRotations[1],
          ModuleConstants.kArmMinBlockedMaxRotations[2] };
    } else {
      m_armSubsystem.armMinMaxRotation = new double[] { ModuleConstants.kArmMinBlockedMaxRotations[0],
          ModuleConstants.kArmMinBlockedMaxRotations[2] };
    }
    if (m_armSubsystem.getArmAngle() < ModuleConstants.kArmMinBlockedMaxRotations[1]
        - ModuleConstants.kArmSafeStandoffRotation) {
      return;
    }*/
    if (elevatorPosition > 0.02) {
      if (elevatorPosition - m < 0) {
        m_elevatorSparkMax0.set(
            MathUtil.clamp(
                m_elevatorPIDController.calculate(elevatorPosition, m), -1, 1)
                * kElevatorMotorSpeedMultiplier);
      }
      if (elevatorPosition - m > 0) {
        m_elevatorSparkMax0.set(
            MathUtil.clamp(
                m_elevatorPIDControllerDown.calculate(elevatorPosition, m), -1, 1)
                * kElevatorMotorSpeedMultiplier);
      }
    } else if (elevatorPosition < 0.02
        && desiredElevatorPosition > 0.02) {
      if (elevatorPosition - m < 0) {
        m_elevatorSparkMax0.set(
            MathUtil.clamp(
                m_elevatorPIDController.calculate(elevatorPosition, m), 0, 1)
                * kElevatorMotorSpeedMultiplier);
      }
      if (elevatorPosition - m > 0) {
        m_elevatorSparkMax0.set(
            MathUtil.clamp(
                m_elevatorPIDControllerDown.calculate(elevatorPosition, m), 0, 1)
                * kElevatorMotorSpeedMultiplier);
      }
    } else {
      if (elevatorPosition - m < 0) {
        m_elevatorSparkMax0.set(
            MathUtil.clamp(
                m_elevatorPIDController.calculate(elevatorPosition, m), 0, 0)
                * kElevatorMotorSpeedMultiplier);
      }
      if (elevatorPosition - m > 0) {
        m_elevatorSparkMax0.set(
            MathUtil.clamp(
                m_elevatorPIDControllerDown.calculate(elevatorPosition, m), 0, 0)
                * kElevatorMotorSpeedMultiplier);
      }
    }
  }
  ...
  public double getElevatorPosition() {
    // if(4 * m_elevatorSparkMax0.getEncoder().getPosition() <
    // m_elevatorAbsoluteEncoder.getPosition())
    // m_elevatorSparkMax0.getEncoder().setPosition(m_elevatorAbsoluteEncoder.getPosition()
    // * 4);
    //System.out.println(Rotation2d.fromRadians(m_elevatorSparkMax0.getEncoder().getPosition()).getRotations()
    //* (1.52*2.54/100*Math.PI) / 6);
    return Rotation2d.fromRadians(m_elevatorSparkMax0.getEncoder().getPosition()).getRotations()
        * (1.52*2.54/100*Math.PI) / 6;
  }

  // returns second stage elevator height (from the base) in meters, calculated
  // only by the absolute encoder
  /*public double getElevatorPositionAbsolute() {
    ...
    if (encoderPosition - lastEncoderPosition < -ModuleConstants.kEncoderResetThreshold)
      encoderRotations += 1;
    else if (encoderPosition - lastEncoderPosition > ModuleConstants.kEncoderResetThreshold)
      encoderRotations -= 1;
  ...
  public void elevateToReef(int reef) {
    MathUtil.clamp(reef, 0, kReefHeights.length - 1);

    setDesiredHeight(kReefHeights[reef]);
  }
